...
三個分頁：**路跑**（9/16 全天 32 站）、**行程**（七天）、**情報**（日文、規則、勘查清單）。
...
（`body.rain`）。勾選 id 加 `r` 前綴（`ra0`、`rb0`），與單車版分開存；P3 的 `c` 兩版共用。
- **門市＝單車版 P1／P2 的子集合、原順序不動**（使用者指定：不換區域、不重排，只砍繞遠的站）。
  `notes/rain_prune.py` 算每站繞路成本（前一站→它→下一站，比直接走多幾公尺；直線×1.3、雨天 65 m/min、每店 2 分），
  >250m 就砍、每砍一家重算；MINISTOP 三家不在優先連鎖直接砍、Lawson 不砍；P2 終點設 JR 浅草橋駅
  （全家一路往南收到 DX）。結果 P1 9 家 5.2 km、P2 與單車版同 6 家 3.1 km。
- `notes/gen_rain.py` 把結果與備註寫進 `stops.js`；座標沿用 `COORDS`。**要改砍站規則改 script 重跑，不要手改。**
- 砍掉的站列在路跑頁第 04 段的雨天版清單，當作「有餘裕就加回」的順序。
...
P1+P2+P3 共 32 站（14＋6＋12），依序對應圖鑑編號 001–032，`SPRITE(n)` 從 PokeAPI/sprites 取圖。
未勾選是灰剪影，勾選後變彩色。站數改了要一起改 hero 的「上午 N 家超商」與 `index.html` 的 `0/32`。
...
4. **P2 全部是全家**，因為全家多數分店 10:00 才開賣，跟 P1 的 07:00 波段錯開。
   2026-09-13 起 P2 改為從秋葉原沿中央通り・昭和通り一路往南收到日本橋（6 家），因為抽到
   寶可夢中心 DX 12:00 的整理券，收完人就在 DX 旁；原本浅草橋 5 家、台東／東上野／御徒町北口
   不再列入（浅草橋在東、DX 在南，兼顧不了）。候選與計算在 `notes/famima_to_dx.py`。
