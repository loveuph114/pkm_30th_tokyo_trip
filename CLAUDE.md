...
兩版都是 14＋11＝25 家，#025 皮卡丘的對應不變。
- 門市全在飯店 400m（P1）／600m（P2）內，2026-09-13 從 Google Maps 網頁版搜尋抓：`notes/rain_candidates.py`
  （候選 64 家、含 cid 與距離）→ `notes/rain_route.py`、`notes/rain_route2.py`（直線×1.3、雨天 65 m/min、
  每店 2 分，Held-Karp 算順序；P1 前四家固定為飯店 200m 內的四家 Lawson）→ `notes/gen_rain.py`
  （備註文字在這裡，寫進 `stops.js`）。**要改路線改 script 重跑，不要手改 RAIN_P1／RAIN_P2。**
- 沒收進來的候選：ローソンストア100（百元店型，不確定有進卡）、ファミマ!! 秋葉原UDX（辦公大樓店型）、
  ファーマライズ薬局末広町（藥局複合店）、ローソン 上野御徒町店（Google 無營業時間、僅 1 則評論，未驗證）。
- 這些門市只驗證了存在與營業時間（非 24h 的查過開店時間），**沒有逐店確認是否進卡**；9/15 勘查若順路可看一眼。
