...
### 整日路線（Google Maps 多點路線）

行程頁每張日卡、路跑頁每個階段，都有「🗺」按鈕開 Google Maps 多點路線，依序標記。
- `COORDS`：站點地圖連結原文 → `"lat,lng"`。**新增門市要一起補座標**，否則該站會被路線略過
  （console 會警告）。座標用瀏覽器開 cid 連結後，從網址的 `!3d<lat>!4d<lng>` 抄。
- `ROUTES`：9/16 以外每天的點位 `[名稱, "lat,lng"]`；9/16 由 P1/P2/P3 + `HOTEL` + `P3_SPLIT`
  自動組成「上午 ①–④／下午 ①②」，刪站不用改路線。
- Google Maps 一段最多 9 個中途點，`app.js` 的 `chunk()` 超過 10 點就切段、首尾共用一點。
