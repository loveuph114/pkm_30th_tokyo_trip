...
- **P1 不沿用單車版**（使用者要求：步行要密集順路，不要有一走好幾分鐘的段）。候選 `notes/kanda_candidates.json`
  是 2026-09-13 從 Google Maps 抓的神田／日本橋／小川町／小伝馬町／岩本町一帶 Lawson・7-11・NewDays（排除辦公大樓內店型、
  ナチュラルローソン）；`notes/rain_p1_dense.py` 以銀座線神田駅為起點、P2 起手店（須田町二丁目全家）為終點錨點，
  前 2 站固定 Lawson，其餘用最便宜插入＋2-opt＋換站挑 14 家，單段超過 350m 的部分加倍罰分。結果 3.3 km、最長一段 390m。
- **P2 與單車版同 6 家**（一路往南走到 DX），`notes/rain_prune.py` 只算步行時程。
- `notes/gen_rain_v2.py` 把兩者與備註寫進 `stops.js`、補 COORDS。**要改路線改 script 重跑，不要手改。**
- 沒收進來的候選列在路跑頁第 04 段的雨天版清單，當作「有餘裕就加碼」的順序。
